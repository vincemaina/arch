...
if not themes:
    ...
    for root, _, files in os.walk(os.path.join(source, "dotfiles")):
        tracked += [f for f in files if f.lower().endswith(
            (".png", ".jpg", ".jpeg",
             # Audio, for the same reason and since TASK-136: the focus noises
             # are generated by ~/.local/bin/focus-noise and cached, and a
             # minute of stereo audio is megabytes. Committing them would grow
             # the repository every time a noise was added, which is exactly
             # when adding one should be free.
             ".opus", ".ogg", ".mp3", ".flac", ".wav", ".m4a"))]
    if tracked:
        say("fail", f"{len(tracked)} image or audio file(s) are tracked in "
                    f"setup/dotfiles/ ({', '.join(sorted(tracked)[:3])}...) - "
                    f"wallpapers and noises are generated now, and committing "
                    f"them grows with every theme or noise added")
    else:
        say("pass", "no images or audio are tracked; a new theme or noise "
                    "costs no bytes")
